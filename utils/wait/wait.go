// SPDX-FileCopyrightText: 2014 Kubernetes.
// SPDX-License-Identifier: Apache-2.0

// Borrowed from k8s.io/apimachinery/pkg/util/wait.
...
import (
	...
	"github.com/seal-io/terraform-provider-courier/utils/clock"
)
...
func runConditionWithCrashProtection(
	...
	return runConditionWithCrashProtectionWithContext(
		context.TODO(),
		condition.WithContext(),
	)
}

// runConditionWithCrashProtectionWithContext runs a
// ConditionWithContextFunc with crash protection.
func runConditionWithCrashProtectionWithContext(
	ctx context.Context,
...
func (b *exponentialBackoffManagerImpl) getNextBackoff() time.Duration {
	if b.clock.Now().
		Sub(b.lastBackoffStart) >
		b.backoffResetDuration {
...
func ExponentialBackoff(
	...
	for backoff.Steps > 0 {
		if ok, err := runConditionWithCrashProtection(condition); err != nil ||
			ok {
			return err
		}
...
func PollInfinite(
	...
	return PollInfiniteWithContext(
		context.Background(),
		interval,
		condition.WithContext(),
	)
...
func PollImmediate(
	...
	return PollImmediateWithContext(
		context.Background(),
		interval,
		timeout,
		condition.WithContext(),
...
func PollImmediateUntil(
	...
	return PollImmediateUntilWithContext(
		ctx,
		interval,
		condition.WithContext(),
	)
...
func PollImmediateInfinite(
	...
	return PollImmediateInfiniteWithContext(
		context.Background(),
		interval,
		condition.WithContext(),
	)
...
// or an error or the specified context gets canceled or expired.
//
...
func poll(
	...
	if immediate {
		done, err := runConditionWithCrashProtectionWithContext(
			ctx,
			condition,
		)
		if err != nil {
